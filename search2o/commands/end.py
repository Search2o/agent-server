...
class EndCommand(CommandExec):
    async def exec_command(self, inv: CommandInvocation, executor: AgentExecutor) -> JsonValue:
        ...
        executor.stream_iter.trace(lambda: f"Ending the agent with the output: {str(val)[:500]}", TraceType.flow, inv.path)
        executor.add_output_part(val)
        raise AgentDone()
